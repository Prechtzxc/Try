...
import { ArrowLeft, ArrowRight, CheckCircle, User, School, Lock, Mail, Eye, EyeOff, AlertTriangle, X, Upload, Loader2, Info, Landmark, FileBadge, UserCheck } from "lucide-react"
...
// Same step-by-step process shown on the "How to Apply" page
const registrationNoticeSteps = [
  {
    number: 1,
    text: "Proceed to the Municipality of Carmona.",
    icon: Landmark,
  },
  {
    number: 2,
    text: "Proceed to the COMELEC Office to secure your Voter's Certificate.",
    icon: FileBadge,
  },
  {
    number: 3,
    text: "Proceed to the CAYDO Office for Registration.",
    icon: UserCheck,
  },
]
                        ...
                        {!emailValidated && (
                          <div className="p-4 sm:p-5 bg-green-50 border border-green-200 rounded-xl shadow-sm">
                            ...
                            <div className="space-y-0">
                              {registrationNoticeSteps.map((stepInfo, index) => {
                                const IconComponent = stepInfo.icon
                                const isLast = index === registrationNoticeSteps.length - 1
                                return (
                                  <div key={stepInfo.number} className="flex gap-3">
                                    <div className="flex flex-col items-center shrink-0">
                                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-green-500 to-emerald-600 text-white border-2 border-white ring-2 ring-green-200 shadow-sm">
                                        <span className="text-xs font-black">{stepInfo.number}</span>
                                      </div>
                                      {!isLast && (
                                        <div className="mt-1.5 w-0.5 flex-1 min-h-[22px] rounded-full bg-green-200" />
                                      )}
                                    </div>
                                    <div className="flex-1 pb-4 pt-1.5 min-w-0">
                                      <div className="flex items-start gap-2">
                                        <IconComponent className="h-4 w-4 shrink-0 mt-0.5 text-green-600" />
                            ...
                            <p className="mt-3 text-xs text-green-700 font-medium leading-relaxed">
                              After your email has been verified, your registered name will automatically appear in the next step.
                            </p>
                          </div>
                        )}
                      </div>
