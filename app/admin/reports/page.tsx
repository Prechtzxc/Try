...
import {
  ...
  MapPin, School, BookOpen, UserIcon, Loader2, Activity,
  FileSpreadsheet, FileImage, CalendarDays, Download, Ban, Banknote, Archive, Accessibility, Search,
  Printer, Eye
} from "lucide-react"
...
import { ReportPrint, type ReportCategory, type ReportScholar } from "@/components/report-print"
...
type ExportReportType = "All" | ReportCategory
...
function filterReportData(cycleData: ReportScholar[], reportType: ExportReportType) {
  if (reportType === "Claimed") {
    return cycleData.filter(s => s.applicationStatus === "claimed" || (s.applicationStatus === "approved" && s.isClaimed))
  }

  if (reportType === "Unclaimed") {
    return cycleData.filter(s => s.applicationStatus === "unclaimed" || (s.applicationStatus === "approved" && !s.isClaimed && !s.isCancelled))
  }

  if (reportType === "Unsuccessful") {
    return cycleData.filter(s => s.applicationStatus === "rejected" || s.applicationStatus === "cancelled")
  }
...
export default function ReportsPage() {
    ...
    const setupRealtimeListeners = () => {
      ...
      const processData = () => {
        ...
        const activeData = allApps.map((app) => {
          ...
          return {
            id: app.id,
            studentId: profile.studentId || app.studentId,
            ...
            rejectionReason: app.cancellationReason || app.feedback || app.remarks || app.rejectionReason || app.resubmissionReason || "N/A",
        ...
        const formattedHistoryRecords = historyData.map(record => {
          ...
          return {
            id: record.id,
            studentId: profile.studentId || record.studentId,
            ...
            rejectionReason: record.cancellationReason || record.feedback || record.remarks || record.rejectionReason || record.resubmissionReason || "N/A",
      ...
      unsubscribeSchedule = onSnapshot(doc(db, "settings", "schedule"), (snap) => {
        setActiveSchedule(snap.exists() ? snap.data() : null);
      });
    };
  ...
  const formatScheduledAmount = (schedule: any) => (
    schedule?.distributionAmount !== undefined && schedule?.distributionAmount !== null && schedule?.distributionAmount !== ""
      ? `₱${schedule.distributionAmount}`
      : "N/A"
  )

  const activeScheduledAmount = formatScheduledAmount(activeSchedule)

  const getHistoricalScheduledAmount = (cycle: string) => {
    const cycleDate = cycle.replace("Cycle Ended: ", "").trim()
    const matchingSchedule = scheduleHistory.find(schedule => formatDate(schedule.endedAt) === cycleDate)
    return formatScheduledAmount(matchingSchedule)
  }

  const generatedBy = user?.name || (user?.profileData as any)?.fullName || user?.email || "Administrator"

  const handlePrintReport = (cycleData: ReportScholar[], cycle: string, scheduledAmount: string) => {
    if (cycleData.length === 0) {
      toast({ title: "No Data", description: `There are no records to print for ${cycle}.` })
      return
    }
    ...
    setPrintRequest({
      ...
      generatedBy,
      scheduledAmount,
      claimed: filterReportData(cycleData, "Claimed"),
      unclaimed: filterReportData(cycleData, "Unclaimed"),
      unsuccessful: filterReportData(cycleData, "Unsuccessful"),
    })

    window.setTimeout(() => window.print(), 300)
  }

  const activeCycleLabel = activeSchedule?.endedAt
    ? `Cycle Ended: ${formatDate(activeSchedule.endedAt)}`
    : "Active Cycle"

  const handlePrintActiveCycle = () => {
    handlePrintReport(activeScholars, activeCycleLabel, activeScheduledAmount)
  }
  ...
  const handleExportExcel = (cycleData: ReportScholar[], exportName: string, reportType: ExportReportType, scheduledAmount: string) => {
    try {
      const filteredData = filterReportData(cycleData, reportType);
            ...
            <Tabs value={activeTab} className="space-y-6" onValueChange={setActiveTab}>
              ...
              <TabsContent value="overview" className="space-y-6">
                <div className="flex justify-between items-center mb-2 gap-3 flex-wrap">
                  ...
                  <Button onClick={handlePrintActiveCycle} aria-label="Print Report" title="Print Report" className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-wide rounded-xl h-10 px-4">
                    <Printer className="h-4 w-4" /> Print Report
                  </Button>
                </div>

                <div id={`pdf-charts-export-Active_Cycle`} className="screen-report-section bg-slate-50/50 p-4 sm:p-6 lg:p-8 rounded-3xl w-full mx-auto max-w-[1100px]">
              ...
              <TabsContent value="demographics" className="space-y-6">
                <div className="flex justify-between items-center mb-2 gap-3 flex-wrap">
                  <h3 className="font-black text-slate-800 uppercase tracking-tight text-xl">Demographics <span className="text-emerald-600">(Active Cycle)</span></h3>
                  <Button onClick={handlePrintActiveCycle} aria-label="Print Report" title="Print Report" className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-wide rounded-xl h-10 px-4">
                    <Printer className="h-4 w-4" /> Print Report
                  </Button>
                </div>
                <div className="screen-report-section">
              ...
              <TabsContent value="academic" className="space-y-6">
                <div className="flex justify-between items-center mb-2 gap-3 flex-wrap">
                  <h3 className="font-black text-slate-800 uppercase tracking-tight text-xl">Academic <span className="text-emerald-600">(Active Cycle)</span></h3>
                  <Button onClick={handlePrintActiveCycle} aria-label="Print Report" title="Print Report" className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-wide rounded-xl h-10 px-4">
                    <Printer className="h-4 w-4" /> Print Report
                  </Button>
                </div>
                ...
                <Card className="rounded-3xl border-slate-200 shadow-sm overflow-hidden bg-white">
                  ...
                  <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <CardDescription className="font-medium text-slate-500 mt-1">Open a cycle's summary with the eye icon, or use the print icon to generate a cycle-specific print list.</CardDescription>
                    </div>
                                    ...
                                    >
                                      ...
                                      <TableCell className="text-right pr-6">
                                        <div className="flex items-center justify-end gap-1.5">
                                          <Button variant="ghost" size="sm" aria-label={`View summary for ${cycleDateStr}`} title="View Summary" onClick={(e) => { e.stopPropagation(); setExpandedCycle(isExpanded ? null : cycle); if (!isExpanded) setSearchQuery(""); }} className="h-9 w-9 rounded-xl text-slate-700 hover:bg-slate-100 hover:text-slate-900">
                                            <Eye className="h-5 w-5" />
                                          </Button>
                                          <Button variant="ghost" size="sm" aria-label={`Print report for ${cycleDateStr}`} title="Print Report" onClick={(e) => { e.stopPropagation(); handlePrintReport(data, cycle, scheduledAmountText) }} className="h-9 w-9 rounded-xl text-slate-700 hover:bg-emerald-50 hover:text-emerald-700">
                                    ...
                                    {isExpanded && (
                                      <TableRow className="bg-slate-50/50 hover:bg-slate-50/50 border-b border-emerald-100">
                                        <TableCell colSpan={6} className="p-0 border-b-0 w-full">
                                          <div className="screen-report-section p-4 sm:p-6 lg:p-8 animate-in slide-in-from-top-2 duration-300 w-full">
                                            ...
                                            <Card className="rounded-3xl border-slate-200 shadow-sm mt-8 bg-white w-full">
